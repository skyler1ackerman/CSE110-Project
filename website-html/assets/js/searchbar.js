...
function getCommunityInfoByName(name){
  ...
  ref.on('value', function(snapshot) {
      ...
      document.getElementById("community-academic-year").innerHTML = communityObj.year
      document.getElementById("community-date-created").innerHTML = communityObj.created
      document.getElementById("community-status").innerHTML = communityObj.status
      document.getElementById("community-organization-type").innerHTML = communityObj.org_type
      document.getElementById("community-purpose").innerHTML = communityObj.purpose
      // document.getElementById("community-discord-link").innerHTML = communityObj  (Pending)
      document.getElementById("community-socialmedia").innerHTML = communityObj.social_media
      document.getElementById("community-email").innerHTML = communityObj.org_email
  });
}
...
function submit_community(){
  ...
  //check if user input is valid
  if(communities.includes(communityInput)){
...
let lastPath = window.location.href.split('/').slice(4)[0]
...
// if (lastPath === "afterlogin.html" || lastPath === "classDB.html") {
//   autocomplete(document.getElementById("inputClasses"), classes);
// }

var communities =[];
autocomplete(document.getElementById("inputCommunities"), communities);
// var communities = ["(APIAH) Asians & Pacific Islanders for the Arts and Humanities",  "A Cappella Choir", "ACM AI", "BioEngineering Newsletter", "Biomedical Engineering Society (BMES)", "Campus Outreach at UCSD"]
// if (lastPath === "afterlogin.html" || window.location.href === "communityDB.html") {
//   autocomplete(document.getElementById("inputCommunities"), communities);
// }
