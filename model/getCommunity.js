    ...
    await ref.once("value", function (snapshot) {
        snapshot.forEach(function (childSnapshot) {
            childSnapshot.forEach(function (clubSnapshot) {
                console.log(clubSnapshot.val().contact);
                if (clubSnapshot.key === communityName) {
                    ...
                    if (clubSnapshot.val().description !== "") {
                        resultsString.str += `<p><b>Description: </b>${
                            clubSnapshot.val().description
                        }</p>`;
                    }
                    if (clubSnapshot.val().social_media !== "") {
                        resultsString.str += `<p><b>Social Media: </b>${
                            clubSnapshot.val().social_media
                        }</p>`;
                    }
                    if (clubSnapshot.val().inviteLink !== "") {
                        ...
                        resultsString.str += `<button class=\"button\" style="margin-left10px; text-align: center; font-family: inherit;" onClick=\"goToReportPageFromCommunity('${
                            clubSnapshot.key
                        }', '${clubSnapshot.val().inviteLink}');\">Report</button>`;
